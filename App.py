...
# =========================
# Hobby canon (from your doc)
# =========================
POT_HOBBY_CANON = {
    "Аметист": {
        "solo": ["Ароматерапия", "Детективные квесты", "Просмотр триллеров"],
        "collective": ["Устроить детективный квест в реальности", "Собрать всех и поиграть в мафию", "Командные игры на интуицию"],
        "recovery": ["Уединение", "Переключение на запахи/сенсорику"],
        "game_goal": ["Выиграть интеллектуальную игру/квест", "Стать сильнее в «Мафии»"],
    },
    "Рубин": {
        "solo": ["Прыжок с парашютом", "Аттракционы", "Гвоздестояние", "Секс (как фокус на ощущениях тела)"],
        "collective": [],
        "recovery": ["Сильный гормональный всплеск и проживание ощущений"],
        "game_goal": ["Сделать эмоциональный вызов/экстрим-план"],
    },
    "Цитрин": {
        "solo": ["Баня", "Расслабляющий массаж", "Тактильные практики", "Поесть с закрытыми глазами", "Ретрит с закрытыми глазами (сенсорика)"],
        "collective": ["Организовать мероприятие", "Найти место", "Составить расписание"],
        "recovery": ["Тактильность, сенсорное восстановление"],
        "game_goal": ["Собрать мини-проект/ивент и довести до результата"],
    },
    "Гелиодор": {
        "solo": ["Дегустация новых вкусов", "Поговорить и обменяться мнениями", "Пофилософствовать"],
        "collective": ["Весь вечер рассказывать истории", "Петь", "Быть тамадой"],
        "recovery": ["Диалог / проговаривание вслух"],
        "game_goal": ["Мини-выступление/история/песня для людей"],
    },
    "Гранат": {
        "solo": ["Просмотр мелодрам", "Мимическое проживание эмоций вместе с героями", "Интерактивный театр"],
        "collective": ["Развлекать всех", "Выражать эмоции", "Стать королём вечеринки"],
        "recovery": ["Эмоциональное проживание и разрядка"],
        "game_goal": ["Сыграть роль/создать эмоцию для людей"],
    },
    "Шунгит": {
        "solo": ["Сходить в зал", "Ощутить напряжение мышц", "Уйти в телесные ощущения (кайф в форме)"],
        "collective": ["Поход в горы с друзьями с палатками"],
        "recovery": ["Телесная нагрузка как переключение головы"],
        "game_goal": ["Пройти маршрут/сделать телесный челлендж"],
    },
    "Сапфир": {
        "solo": ["Музыкальный концерт", "Слушание звуков/голосов", "Поющие чаши", "Астрономия", "Медитация в тишине"],
        "collective": ["Продумать философию мероприятия", "Написать сценарий/квест/головоломку для друзей", "Размышлять с друзьями о смысле жизни"],
        "recovery": ["Тишина / звук / созерцание"],
        "game_goal": ["Создать идею/сценарий/систему смыслов"],
    },
    "Изумруд": {
        "solo": ["Фотосессии", "Выставки картин", "Красота природы", "Сменить одежду", "Перевоплотиться в новый образ"],
        "collective": ["Надеть самый красивый наряд", "Всех накрасить", "Подобрать стиль вечеринки"],
        "recovery": ["Эстетика и визуальная гармония"],
        "game_goal": ["Создать красивый образ/атмосферу для людей"],
    },
    "Янтарь": {
        "solo": ["Архитектура", "Выставка машин", "Изучать детальное строение механизмов", "Выставка роботов"],
        "collective": ["Играть на гитаре у костра"],
        "recovery": ["Порядок/структура/прикладное переключение"],
        "game_goal": ["Создать механизм/систему/прикладный результат"],
    },
}
...
def get_hobby_suggestions_from_p9(p9: list[str]) -> dict:
    """
    Сильный блок хобби:
    - pos4 (позиция 4) -> solo (личные)
    - pos6 (позиция 6) -> collective (коллективные)
    + доп.каналы: recovery + game_goal (из POT_HOBBY_CANON)
    Возвращаем:
      {
        "pos4": "...", "pos6": "...",
        "solo": [...], "collective": [...], "recovery": [...], "game_goal": [...],
        "all_for_today": [...]  # агрегат (как раньше r["hobbies_selected"])
      }
    """
    pos4 = (p9[3] or "").strip() if len(p9) > 3 else ""
    pos6 = (p9[5] or "").strip() if len(p9) > 5 else ""

    # 1) baseline (твои текущие словари)
    solo = []
    collective = []
    ...
    # 2) усиление из POT_HOBBY_CANON (из документа)
    extra4 = (POT_HOBBY_CANON or {}).get(pos4) or {}
    solo += extra4.get("solo", [])
    recovery = extra4.get("recovery", [])
    game_goal = extra4.get("game_goal", [])

    extra6 = (POT_HOBBY_CANON or {}).get(pos6) or {}
    collective += extra6.get("collective", [])

    # чистим
    solo = _uniq(solo)
    collective = _uniq(collective)
    recovery = _uniq(recovery)
    game_goal = _uniq(game_goal)

    all_for_today = []
    all_for_today += [f"Личное: {x}" for x in solo]
    all_for_today += [f"Коллективное: {x}" for x in collective]
    all_for_today += [f"Восстановление: {x}" for x in recovery]
    all_for_today += [f"Игровая цель: {x}" for x in game_goal]
    all_for_today = _uniq(all_for_today)
...
def hobbies_tab(profile: dict):
    ...
    st.markdown("## Хобби — энергия, подпитка и стиль реализации")
    ...
    st.caption(f"Позиция 4 (личное восприятие): **{pos4}** · Позиция 6 (результат/социальность): **{pos6}**")

    # --- текущие сохранённые (совместимость) ---
    saved = r.get("hobbies_selected", []) or []
    # разнесём по категориям (если уже было сохранено раньше)
    def _pref_filter(prefix: str) -> list[str]:
        out = []
        for x in saved:
    ...
    default_solo = _pref_filter("Личное: ")
    default_collective = _pref_filter("Коллективное: ")
    default_recovery = _pref_filter("Восстановление: ")
    default_game = _pref_filter("Игровая цель: ")
    ...
    with c1:
        ...
        solo_sel = st.multiselect(
            "Выберите личные хобби (они подпитывают восприятие/ясность)",
        ...
        rec_sel = st.multiselect(
            "Чем разгружать голову/канал (без давления)",
    ...
    with c2:
        ...
        col_sel = st.multiselect(
            "Коллективные хобби (социальная энергия/результат)",
            options=[f"Коллективное: {x}" for x in sugg["collective"]],
            default=default_collective,
        )

        st.markdown("### 🎯 Игровая цель (мотивация)")
        game_sel = st.multiselect(
            "Что зажигает и тянет «хочу» (как двигатель)",
            options=[f"Игровая цель: {x}" for x in sugg["game_goal"]],
            default=default_game,
        )

    st.write("")
    st.caption("Логика по колонкам 1–2–3: 1) восприятие → личное/восстановление · 2) мотивация → игровая цель · 3) инструмент → коллективное.")

    # собираем обратно в один список (чтобы «Сегодня» работало как раньше)
    combined = _uniq(solo_sel + col_sel + rec_sel + game_sel)
    ...
    st.divider()
    st.markdown("### Быстрый набор на неделю (без перегруза)")
    st.write("Если выбрать минимум: **1 личное + 1 восстановление + 1 игровая цель + 1 коллективное**, неделя становится устойчивее.")
